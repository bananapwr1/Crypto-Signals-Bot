...
- Выполнение сделок на основе сигналов от AI Core
- Бесконечный цикл автоторговли (run_autotrade_cycle)
- Управление стратегиями (Мартингейл, Процентная ставка, Д'Аламбер)

Примечание: Внешние сигналы из парсера используются AI Core для анализа
"""
...
class AutoTrader:
    ...
    async def run_autotrade_cycle(self):
        """
        Бесконечный цикл автоторговли
        Выполняет сделки для пользователей с включенной автоторговлей
        на основе сигналов, сгенерированных AI Core
        Вызывается из main.py через asyncio.gather
        """
        logger.info("🤖 Запуск бесконечного цикла автоторговли...")
        logger.info("📊 Сделки выполняются на основе анализа AI Core")
